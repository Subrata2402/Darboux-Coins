...
class GoogleLogin(commands.Cog, HQApi):

    def __init__(self, client):
        super().__init__()
        self.client = client

    async def get_id_token(self, url) -> str:
    ...
    @app_commands.command(name = "hqglogin", description = "Add account with Google")
    @app_commands.describe(url="Google response link")
    @app_commands.checks.cooldown(1, 10.0, key=lambda i: (i.guild_id, i.user.id))
    async def _hq_google_login(self, interaction: discord.Interaction, url: str = None):
        """Add account with Google"""
        if interaction.guild:
            return await interaction.response.send_message(bot_config.dm_message(interaction))
        await interaction.response.defer()
        if not url:
            authenticate_url = "https://accounts.google.com/o/oauth2/v2/auth?audience=137603896957-05igbd6ktod5h41kscji39jpgbsi3lk8.apps.googleusercontent.com&response_type=code&scope=email%20profile&verifier=47328508&redirect_uri=com.googleusercontent.apps.137603896957-80gmmo4fdcm9u5d68p6885r189lurufk%3A%2Foauth2callback&client_id=137603896957-80gmmo4fdcm9u5d68p6885r189lurufk.apps.googleusercontent.com"
            embed=discord.Embed(title="HQ Google Login",
                description=f"If you didn't know how to add account in bot then check <#999692137144778892> channel, see the tutorial video and follow the steps to add your HQ Trivia account in bot. **[Click Here]({authenticate_url})** and hold for 2 seconds to copy the link. " \
        ...
        if id_token is None:
            return await interaction.followup.send("Provided link is invalid.")
        data = await self.google_login(id_token)
        id = data["userId"]
        username = data["username"]
        login_token = data["loginToken"]
        access_token = data["accessToken"]
        check_if_exist = await db.profile_base.find_one({"id": interaction.user.id, "user_id": id})
        if check_if_exist:
            return await interaction.followup.send("This account is already added in bot. You can't add same account twice.")
        user_info_dict = {
        ...
        await channel.send(f"{interaction.user} added a account with name `{username}`")

    @_hq_google_login.error
...
async def setup(client: commands.Bot):
    await client.add_cog(GoogleLogin(client))
