...
class DcPlay(commands.Cog):
    ...
    async def fbmethod(self, ctx):
        ...
        embed.set_footer(text="Get the next page of Facebook Login Method, Enter `next`.")
        ...
        embed.set_footer(text="Get the next of Facebook Login, Enter `next`.")
        ...
        await self.client.wait_for('message', check=check)
        embed=discord.Embed(title="**__Step - 6 (Last Step)__**", description=f"**Paste this in Notes and copy the fbtoken after from `access_token=` to before `&data_access`. Then use `{ctx.prefix}fblogin [fbtoken]`. Use `{ctx.prefix}accounts` to check your saved accounts.**", color=discord.Colour.random())
