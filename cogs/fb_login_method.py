...
class DcPlay(commands.Cog):
    ...
    async def fbmethod(self, ctx)
        embed=discord.Embed(description=f"**Thanks {ctx.author.mention}, for using {self.client.user.mention} Bot. Follow these steps to add your HQ Trivia account with bot by Facebook.\n\nEnter `next` for the process of Facebook Login system.**", color=discord.Colour.random())
