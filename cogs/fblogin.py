...
class FacebookLogin(commands.Cog(description="Login to HQ Trivia using Facebook"), HQApi):

    def __init__(self, client: commands.Bot):
        super().__init__()
        self.client = client

    @commands.command(aliases=["flink"], description="Get Facebook Login Link")
    @commands.cooldown(1, 10, commands.BucketType.user)
    async def fblink(self, ctx: commands.Context):
    ...
    @commands.command(name="fbverify", aliases=["fblogin", "facebook", "hqflogin", "hqfblogin"], discription="Login to HQ Trivia using Facebook")
    @commands.cooldown(1, 10, commands.BucketType.user)
    async def fbverify(self, ctx: commands.Context, token: str=None):
